...
function updateProjectCounter(parameters: updateToDoCounterParameters) {
  ...
  const counter = document.querySelector<HTMLElement>(`ul#project-list > li#${projectID} .counter`);
  if (!counter) throw new Error(`counter not found for to-do with ID: ${projectID}`);
